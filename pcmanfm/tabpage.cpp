...
void FilterEdit::keyPressed(QKeyEvent *event) {
  ...
  if (!hasFocus() && event->key() != Qt::Key_Left &&
...
TabPage::TabPage(QWidget *parent)
    : QWidget(parent), folderView_{nullptr}, folderModel_{nullptr},
      proxyModel_{nullptr}, proxyFilter_{nullptr}, verticalLayout{nullptr},
      overrideCursor_(false), selectionTimer_(nullptr), filterBar_(nullptr),
      changingDir_(false) {
  ...
  // attach proxy model to the folder view; FolderModel is set as source model
  // in chdir()
  ...
  connect(filterBar_, &FilterBar::textChanged, this,
          &TabPage::onFilterStringChanged);
}
...
TabPage::~TabPage() {
  ...
  if (folderModel_) {
    disconnect(folderModel_, &Fm::FolderModel::fileSizeChanged, this,
               &TabPage::onFileSizeChanged);
    disconnect(folderModel_, &Fm::FolderModel::filesAdded, this,
               &TabPage::onFilesAdded);
    folderModel_->unref();
  }
...
void TabPage::transientFilterBar(bool transient) {
  if (filterBar_) {
    ...
    if (transient) {
      filterBar_->hide();
      connect(filterBar_, &FilterBar::lostFocus, this,
              &TabPage::onLosingFilterBarFocus);
    } else {
      filterBar_->show();
      disconnect(filterBar_, &FilterBar::lostFocus, this,
                 &TabPage::onLosingFilterBarFocus);
    }
  }
}
...
bool TabPage::eventFilter(QObject *watched, QEvent *event) {
  if (watched == folderView_->childView() &&
    ...
    if (QKeyEvent *ke = static_cast<QKeyEvent *>(event)) {
      if (filterBar_ &&
        ...
        // but only emit a signal on pressing a non-autorepeated Backspace if
        // the bar is hidden That signal will be used in MainWindow for going up
        if (ke->key() == Qt::Key_Backspace &&
            ke->modifiers() == Qt::NoModifier && !filterBar_->isVisible()) {
          if (!ke->isAutoRepeat()) {
            Q_EMIT backspacePressed();
          }
        } else if (ke->key() != Qt::Key_Delete) {
          // Don't intercept Delete key - let it be handled by the main window
          // action
  ...
  } else if (watched == folderView_->childView()->viewport() &&
...
void TabPage::onFilterStringChanged(QString str) {
  if (filterBar_ && str != getFilterStr()) {
    ...
    if (transientFilterBar && !filterBar_->isVisibleTo(this) &&
      ...
      if (index.isValid() &&
          !folderView_->selectionModel()->isSelected(index)) {
        filterBar_->clear();
        folderView_->childView()->scrollTo(index,
    ...
    if (transientFilterBar) {
      if (filterBar_->isVisibleTo(
              this)) { // the page itself may be in an inactive tab
        if (str.isEmpty()) {
          // focus the view BEFORE hiding the filter-bar to avoid redundant
          // "FocusIn" events otherwise, another widget inside the main window
          // might gain focus immediately after the filter-bar is hidden and
          // only after that, the view will be focused
...
void TabPage::freeFolder() {
  if (folder_) {
    if (folderSettings_.isCustomized()) {
      ...
      static_cast<Application *>(qApp)->settings().saveFolderSettings(
          folder_->path(), folderSettings_);
    }
    disconnect(folder_.get(), nullptr, this,
               nullptr); // disconnect from all signals
    folder_ = nullptr;
    filesToTrust_.clear();
  }
}
...
void TabPage::onFolderStartLoading() {
  ...
  if (!overrideCursor_) {
    // Some libfm-qt versions can emit extra startLoading() signals without
    // matching finishLoading() Track overrideCursor_ locally so we do not stack
    // multiple override cursors
...
void TabPage::onUiUpdated() {
  ...
  if (!filesToSelect_.empty()) {
    ...
    if (folderView_->selectFiles(infos)) {
      ...
      QModelIndexList indexes =
          folderView_->selectionModel()->selectedIndexes();
      if (!indexes.isEmpty()) {
        folderView_->selectionModel()->setCurrentIndex(
            indexes.first(), QItemSelectionModel::NoUpdate);
      }
    }
  }
  ...
  if (!scrolled) {
    ...
    if (firstIndx.isValid()) {
      folderView_->selectionModel()->setCurrentIndex(
          firstIndx, QItemSelectionModel::NoUpdate);
    }
...
void TabPage::onFolderFinishLoading() {
  ...
  // query filesystem info so free space and total size can be shown and kept up
  // to date
  ...
  // to date For example, the scrollbar ranges are not updated yet. We solve
  // this by installing a Qt timeout handler
  QTimer::singleShot(10, this, &TabPage::onUiUpdated);
}

void TabPage::onFolderError(const Fm::GErrorPtr &err,
                            Fm::Job::ErrorSeverity severity,
                            Fm::Job::ErrorAction &response) {
  if (err.domain() == G_IO_ERROR) {
    if (err.code() == G_IO_ERROR_NOT_MOUNTED &&
        severity < Fm::Job::ErrorSeverity::CRITICAL) {
      auto &path = folder_->path();
      // WARNING: GVFS admin backend has a bug that tries to mount an admin path
      // with a double slash, like "admin://X", even when Admin is already
      // mounted The mount is always completed successfully, so that it can
      // cause an infinite loop here Since "admin" is already handled by
      // canOpenAdmin(), it can be safely excluded here as a workaround
      if (!path.hasUriScheme("admin")) {
        ...
        if (op->wait()) { // blocking event loop, wait for mount operation to
          ...
          // without a matching finishLoading(), so we must restore the busy
          // cursor explicitly because the usual cursor reset will not be
          // triggered
          QApplication::restoreOverrideCursor(); // remove busy cursor
          overrideCursor_ = false;
  ...
  if (folder_ && // the tab may have been closed when waiting for the mount
                 // operation above
    ...
    // Use a modal dialog here so error reporting stays local to the tab even
    // without an info bar
...
QString TabPage::formatStatusText() {
  ...
  // Prefer model row counts over walking the full file list to keep this
  // lightweight
  ...
  auto fi = folder_->info();
  if (fi && fi->isSymlink()) {
    text += QStringLiteral(" (%1)").arg(tr("Link to") + QChar(QChar::Space) +
                                        QString::fromStdString(fi->target()));
  }
...
QString TabPage::pathName() {
  // Use the raw path string instead of displayName() because displayName() can
  // be invalid or localized
  auto filePath = path();
  if (!filePath) {
    return QString();
  }
...
void TabPage::chdir(Fm::FilePath newPath, bool addHistory) {
  ...
  connect(folder_.get(), &Fm::Folder::startLoading, this,
          &TabPage::onFolderStartLoading);
  connect(folder_.get(), &Fm::Folder::finishLoading, this,
          &TabPage::onFolderFinishLoading);

  // connect folder error notifications to TabPage so mount and access errors
  // can be handled centrally
  connect(folder_.get(), &Fm::Folder::error, this, &TabPage::onFolderError);
  connect(folder_.get(), &Fm::Folder::fileSystemChanged, this,
          &TabPage::onFolderFsInfo);
  ...
  connect(folder_.get(), &Fm::Folder::contentChanged, this,
          &TabPage::onFolderContentChanged);

  Settings &settings = static_cast<Application *>(qApp)->settings();
  folderModel_ = CachedFolderModel::modelFromFolder(folder_);
  // always show display names in special places because real names may be
  // unusual (e.g., real names of trashed files may contain trash path with
  // backslash)
  if (!settings.showFullNames() || newPath.hasUriScheme("menu") ||
      newPath.hasUriScheme("trash") || newPath.hasUriScheme("network") ||
      newPath.hasUriScheme("computer")) {
  ...
  proxyModel_->setSortCaseSensitivity(folderSettings_.sortCaseSensitive()
                                          ? Qt::CaseSensitive
                                          : Qt::CaseInsensitive);
...
void TabPage::onSelChanged() {
  ...
  if (folderView_->hasSelection()) {
    ...
    if (numSel ==
        1) { /* only one file is selected (also, tell if it is a symlink) */
      auto &fi = files.front();
      if (!fi->isDir()) {
        QString name =
            static_cast<Application *>(qApp)->settings().showFullNames() &&
      ...
      /* Statusbar plugin hooks similar to the gtk+ version are not implemented
       * here */
    } else {
      goffset sum;
      ...
      if (numSel < 1000) {
        ...
        for (auto &fi : files) {
          if (fi->isDir()) {
            /* directory sizes are not summed here to avoid expensive deep
             * counting */
            sum = -1;
            break;
          }
        ...
        if (sum >= 0) {
          msg += QStringLiteral(" (%1)").arg(
              Fm::formatFileSize(sum, fm_config->si_unit));
        }
        /* additional selection details are intentionally omitted to keep this
         * lightweight */
      }
    }
  }
...
void TabPage::setViewMode(Fm::FolderView::ViewMode mode) {
  ...
  if (prevMode != folderView_->viewMode()) {
    // FolderView::setViewMode() may delete the view to switch between list and
    // tree So, the event filter should be re-installed and the status message
    // should be updated
...
void TabPage::setSortCaseSensitive(bool value) {
  if (proxyModel_) {
    proxyModel_->setSortCaseSensitivity(value ? Qt::CaseSensitive
                                              : Qt::CaseInsensitive);
  }
}
...
void TabPage::saveFolderSorting() {
  ...
  folderSettings_.setSortColumn(
      static_cast<Fm::FolderModel::ColumnId>(proxyModel_->sortColumn()));
...
void TabPage::applyFilter() {
  ...
  if (proxyFilter_->getFilterStr().isEmpty()) {
    ...
    if (curIndex.isValid()) {
      ...
      folderView_->childView()->scrollTo(curIndex,
                                         QAbstractItemView::EnsureVisible);
    } else if (firstIndx.isValid()) {
      // if there is no current item, set the first item as current without
    ...
    if (firstIndx.isValid() &&
      ...
      auto indexList = proxyModel_->match(firstIndx, Qt::DisplayRole,
                                          proxyFilter_->getFilterStr());
    ...
    if (curIndex.isValid()) {
      ...
      if (curIndex.isValid()) {
        folderView_->selectionModel()->setCurrentIndex(
            curIndex, QItemSelectionModel::NoUpdate);
        folderView_->childView()->scrollTo(curIndex,
                                           QAbstractItemView::EnsureVisible);
      }
    }
...
void TabPage::setCustomizedView(bool value, bool recursive) {
  if (folderSettings_.isCustomized() == value &&
      folderSettings_.recursive() == recursive) {
    return;
  }
  ...
  if (value) { // save customized folder view settings
    ...
    // get folderSettings_ again because, although it isn't customized, it may
    // be inherited
    folderSettings_ = settings.loadFolderSettings(path());
    // settings may change temporarily by connecting to the signal
    // TabPage::sortFilterChanged, which will be emitted below (that happens in
    // MainWindow::onTabPageSortFilterChanged, for example), so we should
    // remember its relevant values before proceeding
...
bool TabPage::canOpenAdmin() {
  /* NOTE: "admin:///" requires a special handling because it first needs an
     invisible mount and then the password. Since its password prompt is shown
     with all GIO functions that query file info, a direct call to chdir() will
     show two password prompts if the first one is cancelled.

     Fm::uriExists() (= g_file_query_exists) is used to check Admin. It calls
     the password prompt if Admin is mounted. Four scenarios are possible:

     1. Admin is not supported. Then, the mount operation fails and this method
     returns "false" after showing an error message

     2. Admin is supported but not mounted yet. Then, the first call to
     Fm::uriExists() does not show a password prompt. We mount Admin and ask for
     the password by calling Fm::uriExists() again

     3. If Admin is already mounted but the correct password is not entered yet,
     the password will be asked by the first call to Fm::uriExists(). If the
     password is correct, "true" will be returned; if not,
     MountOperation::wait() will return "false" (because a repeated mount fails)
     and so, another password prompt will not be shown

     4. If Admin is already mounted and the password was entered before, "true"
     will be returned
  */
  ...
  if (folder_) {
    QMessageBox::critical(parentWidget()->window(), QObject::tr("Error"),
                          QObject::tr("Cannot open as Admin"));
  }
  return false;
}
